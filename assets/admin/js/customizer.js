...
(function ( $ ) {
	...
	value.bind(function ( to ) {
	    ...
	    $('.octopus-navbar-default').css('max-width', to + "px");
	});
    });
	...
	value.bind(function ( to ) {
	    if ('' === to) {
		$('.octopus-header-sticky-top .octopus-navbar-wrapper').octopus_sticky_header('destroy');
		$('.site-header').removeClass('octopus-header-sticky-top');
	    } else{
		$('.site-header').addClass(to);
		$('.octopus-header-sticky-top .octopus-navbar-wrapper').octopus_sticky_header();
	    }
	});
    });
...
function updateColorCss ( wp, $colorStyle ) {
    ...
    css += '.octopus-navbar-wrapper {background-color: ' + hexToRgba(bgColor, opacity, true) + ';}';
    css += '.octopus-scrolling .octopus-navbar-wrapper {background-color: ' + hexToRgba(bgColor, opacityOnScroll, true) + ';}';
